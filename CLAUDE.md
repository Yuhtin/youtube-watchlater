...
- `User` owns `Card`s, `Playlist`s, and both sides of `Suggestion`.
- `Card.status` is `ColumnType` enum: `WATCH_LATER` / `WATCHING` / `WATCHED`. `order: Int` is the kanban position within a column.
- `Playlist.playlistId` is the YouTube playlist ID; uniqueness is per `(playlistId, userId)`. Cards optionally reference a `Playlist`.
...
Module-per-domain: `auth/`, `user/`, `card/`, `playlist/`, `suggestion/`, `youtube/`, `cron/`, plus a shared `prisma/` module (global, exports `PrismaService`).
...
**Cron:** `apps/api/src/cron/playlist-resync.service.ts` runs every 6h (toggleable via `PLAYLIST_RESYNC_ENABLED`). Re-fetches every imported playlist, updates titles/durations, removes cards whose videos were deleted on YouTube.

### Client (`apps/web/src/`)

App Router:
- `app/page.tsx` — public landing/user picker.
- `app/login/[userId]/page.tsx` — per-user login.
- `app/watchlater/[userId]/page.tsx` — the main kanban view. ~2900 lines, holds most product logic.
- `components/KanbanBoard.tsx`, `DroppableColumn.tsx`, `SortableItem.tsx` — `@dnd-kit` board primitives.
